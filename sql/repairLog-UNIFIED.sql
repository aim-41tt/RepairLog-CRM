...
BEGIN
    -- Получаем данные заказа, клиента и нового статуса
    SELECT ro.client_id, ro.order_number, c.notifications_enabled, rs.name
    INTO v_client_id, v_order_number, v_notifications_enabled, v_status_name
    FROM repair_orders ro
    JOIN clients c ON c.id = ro.client_id
    JOIN repair_statuses rs ON rs.id = NEW.status_id
    WHERE ro.id = NEW.repair_order_id;

    -- Если заказ, статус или клиент не найдены — безопасно выходим
    IF v_client_id IS NULL OR v_status_name IS NULL THEN
        RETURN NEW;
    END IF;

    -- Если клиент не дал согласия на уведомления — не создаём
    ...
    -- Формируем и вставляем SMS (если шаблон задан)
    IF v_sms_template IS NOT NULL THEN
        v_sms_message := REPLACE(v_sms_template, '{status}', v_status_name);
        v_sms_message := REPLACE(v_sms_message, '#{order_number}', COALESCE(v_order_number, 'N/A'));
        
        INSERT INTO notifications (client_id, repair_order_id, notification_type, message)
        VALUES (v_client_id, NEW.repair_order_id, 'SMS', v_sms_message);
    END IF;

    -- Формируем и вставляем Email (если шаблон задан)
    IF v_email_template IS NOT NULL THEN
        v_email_message := REPLACE(v_email_template, '{status}', v_status_name);
        v_email_message := REPLACE(v_email_message, '#{order_number}', COALESCE(v_order_number, 'N/A'));
        ...
        VALUES (
            ...
            NEW.repair_order_id, 
            'EMAIL', 
            COALESCE(v_email_subject, 'Уведомление') || E'\n\n' || v_email_message
        );
    END IF;
...
INSERT INTO notification_templates (
    ...
    'status_change',
    'Ваш заказ #{order_number} сменил статус на: {status}',
    'Изменение статуса заказа #{order_number}',
    'Уважаемый клиент, статус вашего ремонта обновлен: {status}. Подробности в личном кабинете.',
    TRUE
);
